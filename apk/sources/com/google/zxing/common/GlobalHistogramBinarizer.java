...
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitArray getBlackRow(int y, BitArray row) throws NotFoundException {
        ...
        for (int x = 0; x < width; x++) {
            int pixel = localLuminances[x] & Draft_75.END_OF_FRAME;
            int i = pixel >> 3;
            localBuckets[i] = localBuckets[i] + 1;
        }
        int blackPoint = estimateBlackPoint(localBuckets);
        int left = localLuminances[0] & Draft_75.END_OF_FRAME;
        int center = localLuminances[1] & Draft_75.END_OF_FRAME;
        for (int x2 = 1; x2 < width - 1; x2++) {
            int right = localLuminances[x2 + 1] & Draft_75.END_OF_FRAME;
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (int y2 = 0; y2 < height; y2++) {
            ...
            for (int x2 = 0; x2 < width; x2++) {
                int pixel2 = localLuminances2[offset + x2] & Draft_75.END_OF_FRAME;
